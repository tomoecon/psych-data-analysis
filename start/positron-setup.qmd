---
title: "RとPositronのセットアップ"
description: "RとPositronの導入から、CSVの読み込み、保存したスクリプトの再実行まで"
date: 2026-09-22
format: html
lightbox: true
execute:
...
2. [R-4.6.1 for Windows](https://cran.r-project.org/bin/windows/base/)（英語）を開き、先頭の「Download R-… for Windows」からインストーラー（`.exe`）を保存します。保存したファイルを開き、画面に従ってインストールします。インストール先などは通常、初期設定のままで構いません。
3. [Download Positron](https://positron.posit.co/download.html)（英語）を開き、Windowsのx64用「User install」を保存して、インストールします。公式ページの「Windows setup」に案内されているVisual C++ Redistributableも確認し、未導入ならリンク先のx64用をインストールしてください。
...
2. [R for macOS](https://cran.r-project.org/bin/macosx/)（英語）を開き、「Latest release」の自分のチップに合うインストーラー（`.pkg`）を保存します。R 4.6.1では、Apple Silicon用はmacOS 14以降、Intel用はmacOS 11以降が必要です。条件を満たさない場合は、作業を進める前に教員に相談してください。
3. 保存した`.pkg`を開き、画面に従ってインストールします。ダウンロードフォルダへのアクセスを求められた場合は許可します。
4. [Download Positron](https://positron.posit.co/download.html)（英語）から、同じチップに合うmacOS用ファイル（`.dmg`）を保存します。開いた画面でPositronをApplications（アプリケーション）フォルダへドラッグします。
...
![Positronで使う5つの領域。この後の集計と作図まで終えた画面です。コードはEditorに書き、結果はConsoleやPlotsで確認します。](images/positron-setup/13-layout.png){fig-alt="Explorer、Editor、Console、Variables、Plotsの位置と役割を黄色のラベルで示している。" loading="lazy"}
...
「Start Session」と表示されている場合は、そこをクリックして新しいRを選びます。古い版が動いている場合は、右上のバージョン表示をクリックし、「New Console Session…」から新しいRを選びます。Pythonなど他の言語は選びません。新しいConsoleにRの起動メッセージと `>` が出るまで待ってください。複数のConsoleがある場合は、以後、新しいRのConsoleを使います。Rが一覧に出ない場合は、[Rが見つからない場合](#r-not-found)を確認してください。
...
4. `[1] "R version 4.6.1 (2026-06-24)"` のように表示されます。ダウンロードした版と一致していることを確認しましょう。

この後の手順で「Consoleで実行する」と書かれていたら、同じように入力欄へコードを貼り付け、Enterを押します。結果や説明文は貼り付けず、コードだけを使ってください。

![コードを入力してEnterを押すと、実行したコードの下に結果が表示されます。その下に現れる新しい `>` が、次の入力欄です。](images/positron-setup/03-console-v2.png){fig-alt="①実行したR.version.string、②その実行結果、③次の入力欄 > を、上から順に矢印で示している。" loading="lazy"}
...
![選択したコードが上から順に実行されます。Consoleの上2行は、前の手順で1行だけ実行した結果です。今回の結果は、その下に続く2と6です。](images/positron-setup/06-run-selection.png){fig-alt="Editorで選択した計算2行と、Consoleに表示された結果2と6を示している。" loading="lazy"}

## パッケージの導入 {#sec-packages}
...
![パッケージ読み込み時の案内の例。実行が終わると、一番下に入力待ちの記号が出ます。この撮影では、次の手順で使うCSVもフォルダに保存済みです。](images/positron-setup/07-packages-original.png){fig-alt="Consoleにpsych、lavaan、marginaleffectsの起動メッセージがあり、最後に入力待ちの記号が表示されている。" loading="lazy"}

## CSVの読み込み、集計と作図 {#sec-csv-plot}
...
## 保存したスクリプトの再実行 {#sec-rerun}

コードを保存しておくと、Rを終了した後も同じ処理をやり直せます。Consoleの実行履歴や、Variablesに残ったデータに頼らず、保存したスクリプトから結果を再現できるか確認しましょう。

1. `setup-check.R` を保存し、Editorのタブの×をクリックして閉じ、Explorerから開き直します。CSVの読み込み、行数と列数の確認、年齢の集計、作図のコードが残っていることを確かめます。
...
[第[-@sec-run-r]節（Rの動かし方）](#sec-run-r)の操作を練習します。

まず、次のコードをConsoleの入力欄に貼り付け、Enterを押してください。Consoleにどのような結果が表示されるでしょうか。

```{r}
#| label: setup-exercise-console
12 + 8
```

次に、授業用フォルダに `setup-practice.R` というRスクリプトを作り、次の2行をEditorに貼り付けてください。`*` は掛け算の記号で、括弧の中は先に計算されます。

```{r}
#| label: setup-exercise-editor
6 * 7
(6 + 7) * 2
```

1. 1行目にカーソルを置き、WindowsではCtrl+Enter、macOSではCommand+Enterを押してください。次に、2行目も同じ方法で実行します。
2. 2行をまとめて選択し、同じキー操作で実行してください。Consoleに、1行ずつ実行したときと同じ結果が順に表示されることを確認します。
3. スクリプトを保存してタブを閉じ、Explorerから開き直してください。2行のコードが残っていることを確認します。

ConsoleとEditorでは、コードを実行するキー操作がどう違うか、説明してください。

::: {.callout-note collapse="true" title="問1の解答と確認のポイント"}

Consoleの足し算は20です。Editorの1行目は42、2行目は26になります。2行をまとめて実行すると、42、26の順に結果が表示されます。

ConsoleではEnterで実行します。EditorではEnterは改行で、コードを実行するにはCtrl+Enter（Windows）またはCommand+Enter（macOS）を使います。Editorから実行した場合も、計算結果はConsoleに表示されます。

保存は、コードをファイルに残す操作です。実行しただけでは保存されないことがあるので、ファイルを閉じる前に保存してください。

:::

### 問2：小さなデータを作って集計する

[第[-@sec-csv-plot]節（CSVの読み込み、集計と作図）](#sec-csv-plot)で使った `dim()` と `summary()` を、5人分の架空のテスト得点で試します。今回はCSVを読み込まず、次のコードでデータを作ります。
...
```{r}
#| label: setup-exercise-summary
dim(practice)
summary(practice$score)
```

Consoleの出力を見て、次の問いに答えてください。

1. データは何行・何列でしょうか。
2. 得点の最小値（Min.）、中央値（Median）、平均値（Mean）、最大値（Max.）はいくつでしょうか。
3. 5人の得点を足して5で割り、出力された平均値と一致するか確かめてください。

確認が終わったら、追加したコードも保存してください。

::: {.callout-note collapse="true" title="問2の解答と確認のポイント"}

1. `dim(practice)` の出力は `5 1` で、5行・1列です。1行が1人、1列が得点に対応します。
2. 最小値は60、中央値は80、平均値は76、最大値は90です。
3. 得点の合計は380なので、平均は `380 / 5 = 76` です。Rで確かめる場合は、Consoleで次を実行できます。

```{r}
#| label: setup-exercise-mean-check
(60 + 70 + 80 + 80 + 90) / 5
```

`practice` が見つからないというエラーになった場合は、データを作る3行を先に実行したか確認してください。Editorに貼り付けただけでは、Rの中にデータは作られません。

:::

## 困ったとき {#sec-troubleshooting}
...
Consoleの入力待ちが `>` ではなく `+` のままになったら、括弧や引用符が閉じておらず、Rが続きを待っている可能性があります。Consoleの入力欄をクリックし、文字を選択していない状態でCtrl+Cを押して入力を取り消します。macOSでもCommandではなくcontrolキーを使います。新しい `>` に戻ったら、スクリプトのコードを教材と見比べてから、まとまり全体を実行し直してください。
...
この方法はPNGへの出力です。`ragg` をインストールするだけでは、PositronのPlotsの描画方法やフォント設定が切り替わるわけではありません。保存画像でも文字が出ない場合は、指定したフォントが入っているかを確認してください。
